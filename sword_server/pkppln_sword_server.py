...
def create_deposit(on_behalf_of):
    ...
    for content in contents:
        ...
        insert_success = insert_deposit('add', email.text, deposit_uuid, on_behalf_of,
            checksum_value, content.text, size, 'deposited')
...
def sword_statement(on_behalf_of, deposit_uuid):
    ...
    try:
        with con:
            cur = con.cursor()
            cur.execute("SELECT issue_url FROM archived_issues WHERE deposit_uuid = %s", deposit_uuid)
    except mdb.Error, e:
...
def edit_deposit(on_behalf_of, deposit_uuid):
    ...
    for content in contents:
        ...
        insert_success = insert_deposit('edit', email.text, deposit_uuid, on_behalf_of, checksum_value, content.text, size, 'updated')
...
def insert_deposit(action, email, deposit_uuid, on_behalf_of, checksum_value, url, size, status):
    con = mdb.connect(db_host, db_user, db_password, db_name)
    try:
        with con:
            cur = con.cursor()
            cur.execute("INSERT INTO archived_issues " +
                "(action, journal_manager_email, date_deposited, deposit_uuid, journal_uuid, \
                  sha1_value, issue_url, size, status, harvested, deposited_lom) " +
                "VALUES(%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)", (action, email, datetime.now(),
                deposit_uuid, on_behalf_of, checksum_value, url, size, status, None, None))
        return True
    except mdb.Error, e:
        print "Error %d: %s" % (e.args[0],e.args[1])
        return False
