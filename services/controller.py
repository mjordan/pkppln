...
if len(sys.argv) == 1:
    microservice = sys.argv[0]
else:
    sys.exit('PKP PLN staging server microservices controller called with no arguement')
...
# State request against LOM handled as a separate cron job.
    ...
    sys.exit('PKP PLN staging server microservices controller called with invalid arguement' % sys.argv[0])
